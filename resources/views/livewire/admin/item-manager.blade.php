<div>
                ...
                <tbody>
                    @forelse ($items as $item)
                        <tr wire:key="item-{{ $item->id }}">
                            ...
                            <td>
                                @if ($item->image_path)
                                    <img src="{{ Storage::url($item->image_path) }}" alt="{{ $item->name }}" class="h-10 w-10 object-cover rounded-md">
                                @else
                                    <div class="h-10 w-10 rounded-md bg-slate-200 flex items-center justify-center text-slate-400 text-xs">No Img</div>
                                @endif
                            </td>
                            ...
                            <td>
                                ...
                                @if ($item->sterilization_method)
                                    <div class="mt-1 text-[10px] font-medium text-sky-600 bg-sky-50 px-1.5 py-0.5 rounded w-fit border border-sky-100">
                                        Steril: {{ $item->sterilization_method->label() }}
                                    </div>
                                @endif
                            </td>
    ...
    <x-modal :show="$showForm" :title="$editingId ? 'Ubah Alat' : 'Tambah Alat'">
        <form wire:submit="save" id="item-form" class="space-y-4">
            ...
            <div>
                <label class="field-label" for="item-sterilization">Metode Sterilisasi Default</label>
                <select wire:model="sterilization_method" id="item-sterilization" class="field-input">
                    <option value="">— Tidak ada default —</option>
                    @foreach ($sterilizationOptions as $value => $label)
                        <option value="{{ $value }}">{{ $label }}</option>
                    @endforeach
                </select>
                @error('sterilization_method') <p class="field-error">{{ $message }}</p> @enderror
                <p class="mt-1 text-xs text-slate-500">Sistem akan otomatis menghitung masa kedaluwarsa sesuai metode sterilisasi.</p>
            </div>
            ...
            <div>
                <label class="field-label" for="item-image">Gambar Alat (Opsional)</label>
                <input wire:model="image" id="item-image" type="file" accept="image/*" class="field-input">
                <div wire:loading wire:target="image" class="text-xs text-slate-500 mt-1">Mengunggah...</div>
                @error('image') <p class="field-error">{{ $message }}</p> @enderror
                @if ($image)
                    <div class="mt-2 text-xs text-slate-500">Preview (belum disimpan):</div>
                    <img src="{{ $image->temporaryUrl() }}" class="mt-1 h-20 rounded-md border border-slate-200">
                @elseif ($currentImage)
                    <div class="mt-2 text-xs text-slate-500">Gambar saat ini:</div>
                    <img src="{{ Storage::url($currentImage) }}" class="mt-1 h-20 rounded-md border border-slate-200">
                @endif
            </div>
